...
#if 0
	uint8_t val;
   val = IsEmptyCanQueueTx();
	return !val;
#else
	extern CAN_HandleTypeDef hcan1;
	extern int dev_txempty(CAN_HandleTypeDef *hcan);

	return dev_txempty(&hcan1);
#endif	
}
// -----------------------------------------------------------------------------
  ...
  *msg++ = msCAN12.RxBuf.DataFld[7];
  ...
  return 0;
	...
	if(1 == GetCan1QueueRx(head, &RxMessage))
	{
		...
		*msg++ = RxMessage.data[2];
		*msg++ = RxMessage.data[3];
		*msg++ = RxMessage.data[4];
		*msg++ = RxMessage.data[5];
		*msg++ = RxMessage.data[6];
		*msg++ = RxMessage.data[7];

		cob_id = RxMessage.cob_id;
		if(cob_id == g_ccp_cro_id)
		{
			return 1;
		}
		else
