...
typedef int CCP_TX_HANDLE_STR;
CCP_TX_HANDLE_STR CCP_TX_HANDLE;

#define C_SINGLE_RECEIVE_BUFFER
#define C_ENABLE_TRANSMIT_QUEUE_0
#define CCP_CAN_CHANNEL  0
#define kCanTxOk 0


int CanTransmit_0(CCP_TX_HANDLE_STR ccp_tx_handle)
{
	extern uint8_t Can_Send(CAN_PORT notused, Message *m, CAN_HandleTypeDef *hcan);
	extern unsigned int g_ccp_dto_id;              // global DTO-ID
	...
	result = Can_Send((CAN_PORT)NULL, &m, &hcan1);
	if(HAL_OK == result)
	{
		return kCanTxOk;
	}
...
{
	extern CCP_BYTE CCP_TX_DATA_PTR[8]; 
...
void eeWrite(CCP_MTABYTEPTR addr, CCP_BYTEPTR data, CCP_BYTE size)
{
	return;
}
