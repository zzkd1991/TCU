...
uint32_t SPI_FLASH_ReadDeviceID(void)
{
	...
	uint8_t W25X_DeviceID = 0x90;
	uint8_t Dummy_Byte	= 0xFF;
	uint8_t zero_byte = 0;
	//extern void spi3_set_speed(uint8_t speed);
	...
	SPI_FLASH_CS_LOW();
#if 0
	spi3_trx(1, &W25X_DeviceID, &Temp);
	//spi3_trx(4, (uint8_t *)&Dummy_Byte, (uint8_t *)&Temp);
	spi3_trx(1, &zero_byte, &Temp);
	spi3_trx(1, &zero_byte, &Temp);
	spi3_trx(1, &zero_byte, &Temp);

	spi3_trx(1, &Dummy_Byte, (uint8_t *)&Temp);
	...
	deviceid |= Temp;
#else
    spi3_read_write_byte(W25X_DeviceID);
    spi3_read_write_byte(0);
    spi3_read_write_byte(0);
    spi3_read_write_byte(0);
    deviceid = spi3_read_write_byte(0xFF) << 8;
    deviceid |= spi3_read_write_byte(0xFF);
...
uint32_t SPI_FLASH_ReadID(void)
{
  ...
  uint8_t W25X_JedecDeviceID = 0x9F;
  ...
  spi3_trx(1, &W25X_JedecDeviceID, NULL);

  spi3_trx(1, &Dummy_Byte, (uint8_t *)&Temp0);

  spi3_trx(1, &Dummy_Byte, (uint8_t *)&Temp1);

  spi3_trx(1, &Dummy_Byte, (uint8_t *)&Temp2);
