...
uint16_t Vol_Set_Value = 0;

TIM_HandleTypeDef TIM6_TimeBaseStructure;
...
{
	extern float ch1_get_oilpres_value(void); //COV_RWV_ABPresSens_FORSDO

	float oilpres;
	oilpres = ch1_get_oilpres_value();
	
	return (uint16_t) (oilpres * 1000);
}
...
uint8_t AO_Diag_State_Get(void)
{
	...
	actual_value = API_GET_AO_Voltage();

	diff_value = abs((double)(Vol_Set_Value - actual_value));

	if(diff_value <= Vol_Set_Value * 0.05)
	{
		return 1;
	}
	else
