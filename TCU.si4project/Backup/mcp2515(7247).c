...
static const struct can_bittiming;
...
static uint8_t mcp2515_read_reg(uint8_t reg)
{
	...
	spi_trx(2, spi_tx_buf, NULL);

	spi_trx(1, NULL, &spi_rx_buf);
...
{
	spi_trx(TXBDAT_OFF + len, buf, NULL);
}
...
static void mcp2515_hw_tx(Message* m, int tx_buf_idx)
{
	...
	spi_trx(1, &INSTRUCTION_RTS(1 << tx_buf_idx), NULL);
...
static int mcp2515_set_normal_mode(uint32_t ctrlmode)
{
	...
	else (ctrlmode & CAN_CTRLMODE_LISTENONLY)
	{
...
static int mcp2515_hw_reset(struct )
{
	int ret;
	uint8_t spi_tx_buf;
...
static int mcp2515_can_ist(enum can_state *state)
{
	int force_quit = 0;
