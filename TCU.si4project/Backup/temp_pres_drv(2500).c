...
static void Rheostat_ADC_Mode_Config(void)
{
	...
	HAL_ADCEx_MultiModeConfigChannel(&ADC_Handle, &mode);

	HAL_ADC_Start(&_ADC_Handle);

	__HAL_LINKDMA(&ADC_Handle, DMA_Handle, DMA_Init_Handle);
	__HAL_LINKDMA(&_ADC_Handle, DMA_Handle, DMA_Init_Handle);
	HAL_ADCEx_MultiModeStart_DMA(&ADC_Handle, (uint32_t *)ADC_ConvertedValue, 18);
}
...
#define ADC_CONVERTED_CHANNEL_NUM	18
