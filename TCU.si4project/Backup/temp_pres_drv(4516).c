...
static void Rheostat_ADC_Mode_Config(void)
{
	...
	ADC_Config.Channel = RHEOSTAT_ADC_CHANNEL1;//Vol_Drive1_Cur1(PC1比例电磁铁输出电压放大)
	ADC_Config.Rank = 1;
	//采样时间间隔为480个时钟周期
	...
	ADC_Config.Channel = RHEOSTAT_ADC_CHANNEL2;//Vol_Drive1_Cur2(PC1比例电磁铁输出电压放大)
	ADC_Config.Rank = 2;
	if(HAL_ADC_ConfigChannel(&ADC_Handle, &ADC_Config) != HAL_OK)
	{
		Error_Handler();
	...
	ADC_Config.Channel = RHEOSTAT_ADC_CHANNEL3;//Vin_ADC(PC2采样电源电压)
	ADC_Config.Rank = 3;
	if(HAL_ADC_ConfigChannel(&ADC_Handle, &ADC_Config) != HAL_OK)
	{
		Error_Handler();
	...
	ADC_Config.Channel = RHEOSTAT_ADC_CHANNEL4;//EX_CH1(PC3温压一体传感器)
	ADC_Config.Rank = 4;
	if(HAL_ADC_ConfigChannel(&ADC_Handle, &ADC_Config) != HAL_OK)
	{
		Error_Handler();
	...
	ADC_Config.Channel = RHEOSTAT_ADC_CHANNEL5;//EX_CH2(PC4温压一体传感器)
	ADC_Config.Rank = 5;
	if(HAL_ADC_ConfigChannel(&ADC_Handle, &ADC_Config) != HAL_OK)
	{
		Error_Handler();
	...
	ADC_Config.Channel = RHEOSTAT_ADC_CHANNEL6;//EX_CH3(PC5温压一体传感器)
	ADC_Config.Rank = 6;
	if(HAL_ADC_ConfigChannel(&ADC_Handle, &ADC_Config) != HAL_OK)
	{
		Error_Handler();
	...
	ADC_Config.Channel = RHEOSTAT_ADC_CHANNEL7;//EX_CH4(PB0温压一体传感器)
	ADC_Config.Rank = 7;
	if(HAL_ADC_ConfigChannel(&ADC_Handle, &ADC_Config) != HAL_OK)
	{
		Error_Handler();
	...
	ADC_Config.Channel = RHEOSTAT_ADC_CHANNEL8;//Vol_Drive2_Cur1(PC2比例电磁铁输出电压放大)
	ADC_Config.Rank = 8;
	if(HAL_ADC_ConfigChannel(&ADC_Handle, &ADC_Config) != HAL_OK)
	{
		Error_Handler();
	...
	ADC_Config.Channel = RHEOSTAT_ADC_CHANNEL9;//Vol_Drive2_Cur2(PC2比例电磁铁输出电压放大)
	ADC_Config.Rank = 9;
	if(HAL_ADC_ConfigChannel(&ADC_Handle, &ADC_Config) != HAL_OK)
	{
		Error_Handler();
...
void oilpres_set(void)
{
	extern UNS16 COV_RWV_ABPresSens_FORSDO;
	extern UNS16 COV_RWV_TPresSens_FORSDO;
	extern UNS16 COV_RFV_PresSens;

	COV_RWV_ABPresSens_FORSDO = (UINT16)ch1_get_oilpres_value();
	COV_RWV_TPresSens_FORSDO = (UINT16)ch2_get_oilpres_value();
	COV_RFV_PresSens = (UINT16)ch3_get_oilpres_value();

	//ch4_oilpres = (UINT16)ch4_get_oilpres_value();
}
