...
void TLE7242_GPIO_Init(void)
{
  ...
  GPIO_InitTypeDef GPIO_InitStruct = {0};
	...
	HAL_GPIO_WritePin(TLE7242_ENABLE_PORT, TLE7242_ENABLE_PIN, GPIO_PIN_SET);
	
	HAL_GPIO_WritePin(TLE7242_RESET_PORT, TLE7242_RESET_PIN, GPIO_PIN_SET);
...
void TLE_Write_Register_Data(uint8_t channel_u8,uint8_t mess_id_u8,uint32_t SPI_Data_u32)
{
	...
	temp |= 1 << 31;
	temp |= (mess_id_u8 & 0x1f) << 25;
	temp |= (channel_u8 & 0x03) << 23;
	temp |= SPI_Data_u32;

	spi_trx(4, (uint8_t *)&temp, NULL);
...
uint32_t TLE_Read_Register_Data(uint8_t channel_u8,uint8_t mess_id_u8)
{
	...
	spi_trx(4, (uint8_t *)&temp, (uint8_t *)&result);
